...
const AnalysisResult: React.FC<AnalysisResultProps> = ({ data, repo }) => {
  ...
  const getFullMarkdown = () => {
    return `# Analysis: ${repo.owner}/${repo.repo}
...
## Learning Path
${data.learningPath?.steps.map((s, i) => `${i + 1}. **${s.title}**: ${s.description}${s.file ? ` (See \`${s.file}\`)` : ''}`).join('\n') || 'N/A'}
...
---
*Generated by [RepoLens](https://github.com/Dsp023/repolens) - AI-Powered Codebase Analysis*
`;
  };

  const exportToMarkdown = () => {
    const mdContent = getFullMarkdown();
    ...
    <div className="w-full max-w-5xl mx-auto pb-12 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
      ...
      <div className="mb-6 px-1 flex flex-col md:flex-row md:items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-4 text-sm text-fg-muted">
          ...
          <button
            onClick={copyDocForReadme}
            className="px-2 py-0.5 rounded-full bg-canvas-subtle border border-border-default text-xs font-medium text-fg-default hover:bg-canvas-default transition-colors flex items-center gap-1.5"
          >
            <CopyIcon className="w-3 h-3" />
            <span>{copiedDoc ? 'Copied!' : 'Copy for README'}</span>
          </button>
        </div>
      </div>
